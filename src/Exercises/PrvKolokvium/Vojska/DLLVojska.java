package Exercises.PrvKolokvium.Vojska;
...
public class DLLVojska {
    ...
    private static DLL<Integer> vojska(DLL<Integer> lista, int a, int b, int c, int d) {
        ...
        while(temp != null){
            if(temp.element == a) A = temp;
            if(temp.element == b) B = temp;
            if(temp.element == c) C = temp;
            if(temp.element == d) D = temp;
            temp = temp.succ;
        }
        temp = lista.getFirst ();

        while(temp != A){
            result.insertLast (temp.element);
            temp = temp.succ;
        }
