package Exercises.VtorKolokvium.Patarini;
...
public class Paytolls {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Graph<Integer> graph = new Graph<Integer> (brojGradovi);

        for(int i=0;i<brojGradovi;i++)
            graph.adjList[i].setInfo(i+1);


        for(int i=0; i<brojPatista; i++){
            String line1 = br.readLine ();
            String[] parts1 = line1.split (" ");
            graph.addEdge (Integer.parseInt (parts1[0])-1,
                    Integer.parseInt (parts1[1])-1,
                    Integer.parseInt (parts1[2]));
        }

        float[] distance = graph.dijkstra (from-1);
        System.out.println (((int) distance[to - 1]));
    }
