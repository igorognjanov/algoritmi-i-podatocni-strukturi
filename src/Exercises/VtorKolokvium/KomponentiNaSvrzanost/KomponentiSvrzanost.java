package Exercises.VtorKolokvium.KomponentiNaSvrzanost;
...
class GraphNode<E> {
    ...
    public GraphNode(int index, E ime) {
        this.index = index;
        this.info = ime;
...
class Graph<E> {
    ...
    int [] najdikomponenti(int start){
        ...
        int[] niza = new int[adjList.length];

        boolean visited[] = new boolean[num_nodes];
        for (int i = 0; i < num_nodes; i++)
            visited[i] = false;
        visited[start] = true;
        //System.out.println(start+": " + adjList[start].getInfo());
        ...
        sort (niza1, niza1.length);
        return niza1;
    }

    public static void sort(int [] niza, int n){
        for(int i=0; i<n-1; i++){
