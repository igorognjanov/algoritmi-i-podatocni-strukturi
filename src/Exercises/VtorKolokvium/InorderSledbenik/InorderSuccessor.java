package Exercises.VtorKolokvium.InorderSledbenik;
...
class BinarySearchTree<E extends Comparable<E>> {
    ...
    public E getSuccessor(BNode<E> t, E x, BNode<E> parent) {
        // vasiot kod ovde
        if(t == null) return parent.info;
        if(t.info == x){
            if(t.right != null){
        ...
        if(x.compareTo (t.info) < 0){
            return getSuccessor (t.left, x, t);
        }
        return getSuccessor (t.right, x, parent);

    }

    public E getSuccessor(E x) {
        E s = getSuccessor(root, x, root);
        if (s == null) return root.info;
        return s;
