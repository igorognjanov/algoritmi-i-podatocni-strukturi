package Exercises.VtorKolokvium.VkupenBrojNaPatista;
...
class Graph<E> {
    ...
    public int pathCount(int nodeIndex,int pathLength){
        //vasiot kod tuka
        return pathCountR (nodeIndex, 0, pathLength);

    }
    public int pathCountR(int i, int counter, int pathLength){
        if(counter == pathLength) return 1;
        int c = 0;
        for(int j=0; j<adjMat.length; j++){
            if (i==j)continue;
            if(adjMat[i][j] == 1){
                c += pathCountR (j, counter + 1, pathLength);
            }
        }
        return c;
    }
